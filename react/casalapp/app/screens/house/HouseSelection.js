...
import { auth, db } from '../../services/firebase';// auth = autenticação do firebase
...
const HouseSelection = () => {
  ...
  const handleJoinHouse = async () => {
     ...
     */
       ...
       */
      const houseSnap = await getDoc(houseRef);// o get busca o documento do banco
  
      if (!houseSnap.exists()) {
        Alert.alert('Erro', 'Casa não encontrada.');//verifica se o documento não é vazio, ou seja, se ja existe casa com o id
        return;
      }
      ...
      await updateDoc(houseRef, {
        members: arrayUnion(user.uid), //adiciona o membro a casa
      });
